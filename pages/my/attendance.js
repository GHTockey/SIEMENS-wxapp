...
Page({
  ...
  selectActivity(e) {
    ...
    this.data.attendanceList[index].selected = !this.data.attendanceList[index].selected;
    this.setData({
  ...
  selectAll() {
    ...
    this.data.attendanceList.forEach(item => {
    ...
    deleteBtnHandler() {
      ...
      // 判断是否选择了数据
      const selectedList = this.data.attendanceList.filter(item => item.selected);
